#
# NSTEMI: angiogram eligibility: Key models 
#
# K. Fleetwood
# 7 Feb 2025
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_nstemi_angel")
...
cohort <- 
  ...
  mutate(
    ...
    angiogram_eligibility = factor(angiogram_eligibility, levels = c("No", "Yes")),
...
mod_1 <-
  glmer(
    angiogram_eligibility ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    angiogram_eligibility ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    angiogram_eligibility ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5_bin <-
  glmer(
    angiogram_eligibility ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
# Re-start, set start values based on final values from initial run
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "10a_care_nstemi_angel_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "10a_care_nstemi_angel_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "10a_care_nstemi_angel_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "10a_care_nstemi_angel_anova_int.csv")
) 
