#
# NSTEMI: angiogram within 72 hours: Key models 
#
# K. Fleetwood
# 9 Feb 2025
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_nstemi_ang72")
...
cohort <- 
  ...
  mutate(
    ...
    angio_within_72h = factor(angio_within_72h, levels = c("No", "Yes")),
...
mod_1 <-
  glmer(
    angio_within_72h ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    angio_within_72h ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    angio_within_72h ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5_bin <-
  glmer(
    angio_within_72h ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
mod_5_bin_restart_start <- Sys.time()
mod_5_bin_restart <-
  update(
    mod_5_bin,
    start = start_vals,
...
mod_5_bin_restart_finish <- Sys.time()
mod_5_bin_restart_time <- difftime(mod_5_bin_restart_finish,mod_5_bin_restart_start, units = "mins")

mod_5_bin_out <- glmer_out(mod_5_bin_restart, prefix = "mod5_bin_")
...
# *** Initially failed to converge
# Re-start using final values from initial model
start_vals <- getME(mod_5_int, c("theta", "fixef"))
start_vals$theta <- runif(1, start_vals$theta - 0.05*abs(start_vals$theta), start_vals$theta + 0.05*abs(start_vals$theta))
start_vals$fixef <- runif(length(start_vals$fixef), start_vals$fixef - 0.05*abs(start_vals$fixef), start_vals$fixef + 0.05*abs(start_vals$fixef))

mod_5_int_restart_start <- Sys.time()
mod_5_int_restart <-
  update(
    mod_5_int,
    start = start_vals,
...
mod_5_int_restart_finish <- Sys.time()
mod_5_int_restart_time <- difftime(mod_5_int_restart_finish,mod_5_int_restart_start, units = "mins")

mod_5_int_out <- glmer_out(mod_5_int_restart, prefix = "mod5_int_")
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "12c_care_nstemi_ang72_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "12c_care_nstemi_ang72_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "12c_care_nstemi_ang72_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "12c_care_nstemi_ang72_anova_int.csv")
) 
