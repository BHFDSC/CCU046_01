#
# NSTEMI: receipt of all secondary prevention medication: Key models 
#
# K. Fleetwood
# 10 Feb 2025: Adapted from 14c
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_nstemi_prev")
...
cohort <- 
  ...
  mutate(
    ...
    prev_all = factor(prev_all, levels = c("No", "Yes")),
...
mod_1 <-
  glmer(
    prev_all ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    prev_all ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    prev_all ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
# *** Initially failed on converge
# Repeat using NLOPT_LN_NELDERMEAD
mod_5_start <- Sys.time()
mod_5_start
mod_5_v2 <-
  glmer(
    ...
    control = 
      glmerControl(
        optimizer = "nloptwrap", 
        optCtrl = list(algorithm="NLOPT_LN_NELDERMEAD", maxeval = 45000, xtol_rel = 1e-10)
      )
  )
mod_5_finish <- Sys.time()
mod_5_time <- mod_5_finish - mod_5_start
...
# Summarise results
mod_5_out <- glmer_out(mod_5_v2, prefix = "mod5_")
...
mod_5_bin <-
  glmer(
    prev_all ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "15a_care_nstemi_prev_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "15a_care_nstemi_prev_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "15a_care_nstemi_prev_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "15a_care_nstemi_prev_anova_int.csv")
) 
