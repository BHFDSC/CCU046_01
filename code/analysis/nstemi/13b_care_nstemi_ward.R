#
# NSTEMI: admission to cardiac ward: Key models 
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_nstemi_ward")
...
cohort <- 
  ...
  filter(
    mi_type %in% "NSTEMI",
    admission_to_cardiac_ward %in% c("Yes", "No")
  ) %>%
  mutate(
    smi = 
      factor(
    ...
    admission_to_cardiac_ward = factor(admission_to_cardiac_ward, levels = c("No", "Yes")),
...
mod_1 <-
  glmer(
    admission_to_cardiac_ward ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    admission_to_cardiac_ward ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    admission_to_cardiac_ward ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
# **** Initially didn't converge
# Re-start using final values from initial model
start_vals <- getME(mod_4, c("theta", "fixef"))
...
mod_4_restart_finish <- Sys.time()
mod_4_restart_time <- difftime(mod_4_restart_finish, mod_4_restart_start, units = "mins")

mod_4_out <- glmer_out(mod_4_restart, prefix = "mod4_")
...
mod_4_bin <-
  glmer(
    admission_to_cardiac_ward ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + (1|procode3),
...
# *** Initially didn't converge
# Re-start using final values from initial model
start_vals <- getME(mod_4_bin, c("theta", "fixef"))
start_vals$theta <- runif(1, start_vals$theta - 0.05*abs(start_vals$theta), start_vals$theta + 0.05*abs(start_vals$theta))
start_vals$fixef <- runif(length(start_vals$fixef), start_vals$fixef - 0.05*abs(start_vals$fixef), start_vals$fixef + 0.05*abs(start_vals$fixef))

mod_4_bin_restart_start <- Sys.time()
mod_4_bin_restart <-
  update(
    mod_4_bin,
    start = start_vals,
...
mod_4_bin_restart_finish <- Sys.time()
mod_4_bin_restart_time <- difftime(mod_4_bin_restart_finish, mod_4_bin_restart_start, units = "mins")

mod_4_bin_out <- glmer_out(mod_4_bin_restart, prefix = "mod4_bin_")
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "13f_care_nstemi_ward_mod1to4.csv")
)
write.csv(
  anova_1to4, 
  file.path(outcome_folder, "13f_care_nstemi_ward_anova1to4.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "13f_care_nstemi_ward_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "13f_care_nstemi_ward_anova_int.csv")
) 
