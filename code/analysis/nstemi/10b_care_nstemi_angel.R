...
# Create binary SMI variable for model 7
cohort <-
  cohort %>%
  mutate(
    smi_bin =
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "10f_care_nstemi_angel_mod1to4.csv")
)
write.csv(
  anova_1to4, 
  file.path(outcome_folder, "10f_care_nstemi_angel_anova1to4.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "10f_care_nstemi_angel_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "10f_care_nstemi_angel_anova_int.csv")
) 
