...
flow_tbl <- 
  tbl(
    con, 
    in_schema("", "ccu046_01_tmp_inc_exc_flow")
  )
...
cohort_tbl <- 
  tbl(
    con, 
    in_schema("", "ccu046_01_out_cohort_combined")
  )
...
not_in_gdppr <- sum(!cohort$in_gdppr)
...
sum_mal_mi_dod <- sum(cohort$in_gdppr & cohort$mal_mi_dod)

# Exclude people with MI date in March 2023
sum_mar_2023 <- 
  sum(
    cohort$in_gdppr & 
      !cohort$mal_mi_dod & 
...
flow_out <- 
  ...
  bind_rows(
    data.frame(
      df_desc = c("post exclusion of patients not in GDPPR","post exclusion of patients with date of death before mi date", "post exlusion of patients with MI in March 2023"),
      n = c(nrow(cohort) - not_in_gdppr, nrow(cohort) - not_in_gdppr - sum_mal_mi_dod, nrow(cohort) - not_in_gdppr - sum_mal_mi_dod - sum_mar_2023) 
    )
  ) %>%
  mutate(
    n = 5*round(n/5)
...
# Check MI date
sum(is.na(cohort$mi_event_date)) 
summary(cohort$mi_event_date) 

# Check date of birth
sum(is.na(cohort$dob)) 

# Check date of death
sum(!is.na(cohort$dod)) 
summary(cohort$dod)

# Check consistency between dates
dim(cohort)
sum(cohort$dob < cohort$mi_event_date) 
sum(!is.na(cohort$dod) & (cohort$dod > cohort$dob))
sum(!is.na(cohort$dod) & (cohort$dod >= cohort$mi_event_date)) 

# 4.3 Define variables --------------------------------------------------------

# 4.3.1 Severe mental illness -------------------------------------------------
...
# 4.3.2 Additional sociodemographic characteristics ---------------------------
...
# 4.3.5 Lifestyle factors (smoking and BMI) -----------------------------------
...
# 4.3.6 Existing CVD (previous MI, angina) ------------------------------------
...
# 4.3.10 Add discharge destination --------------------------------------------
cohort <- 
  cohort %>%
  mutate(
