#
# STEMI: door to balloon withing 90 minutes: Key models 
#
# K. Fleetwood
# 18 Feb 2025
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_stemi_dtb90")
...
mod_1 <-
  glmer(
    dtb_90 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    dtb_90 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    dtb_90 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5_bin <-
  glmer(
    dtb_90 ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "23a_care_stemi_dtb90_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "23a_care_stemi_dtb90_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "23a_care_stemi_dtb90_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "23a_care_stemi_dtb90_anova_int.csv")
) 
