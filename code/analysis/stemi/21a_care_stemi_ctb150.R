#
# STEMI: call to balloon within 150 minutes: Key models 
#
# K. Fleetwood
# 17 Feb 2025 
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_stemi_ctb150")
...
mod_1 <-
  glmer(
    ctb_150 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    ctb_150 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    ctb_150 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5_bin <-
  glmer(
    ctb_150 ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "21a_care_stemi_ctb150_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "21a_care_stemi_ctb150_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "21a_care_stemi_ctb150_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "21a_care_stemi_ctb150_anova_int.csv")
) 
