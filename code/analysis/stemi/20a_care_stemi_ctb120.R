#
# STEMI: call to balloon within 120 minutes: Key models 
#
# K. Fleetwood
# 17 Feb 2025
#
...
outcome_folder <- file.path(ccu046_01_folder, "processed_data_stemi_ctb120")
...
mod_1 <-
  glmer(
    ctb_120 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex  + (1|procode3),
...
mod_2 <-
  glmer(
    ctb_120 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + 
...
mod_3 <-
  glmer(
    ctb_120 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
...
mod_5 <-
  glmer(
    ctb_120 ~ smi + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      ...
      cardiac_arrest + covid + cardiogenic_shock + 
...
mod_5_bin <-
  glmer(
    ctb_120 ~ smi_bin + age_mi_scl + I(age_mi_scl^2) + sex + imd_2019 + ethnic_5 +
      period_4 + mi_wd + mi_on + comorb_angina + comorb_mi +
      hx_pci +
      comorb_hf +  comorb_diab +
      comorb_crf + comorb_cevd + 
...
saveRDS(mod_5_int, file = file.path(outcome_folder, "20a_mod5_int.rds"))
# mod_5_int <- readRDS(file = file.path(outcome_folder, "20a_mod5_int.rds"))

# Estimate MI OR for each period

coefeq <- 
  matrix(
    data = 0,
    nrow = 10, 
    ncol = nrow(summary(mod_5_int)$coef)
  )
colnames(coefeq) <- row.names(summary(mod_5_int)$coef)
...
mod_5_int_mi_period_ci <-
  confint(
    mod_5_int_mi_period,
    calpha = univariate_calpha()
  )

mod_5_int_mi_period_ci_out <-
  as.data.frame(mod_5_int_mi_period_ci$confint) %>%
  mutate(
  ...
  dplyr::select(period, starts_with("or"))

write.csv(
  mod_5_int_mi_period_ci_out, 
  file = file.path(outcome_folder, "20a_care_stemi_ctb120_mod_int_lc.csv")
)
...
write.csv(
  mod_out, 
  file = file.path(outcome_folder, "20a_care_stemi_ctb120_mod1to5.csv")
)
write.csv(
  anova_1to5, 
  file.path(outcome_folder, "20a_care_stemi_ctb120_anova1to5.csv")
)
...
write.csv(
  mod_out_int, 
  file = file.path(outcome_folder, "20a_care_stemi_ctb120_mod_int.csv")
)
...
write.csv(
  anova_int,
  file = file.path(outcome_folder, "20a_care_stemi_ctb120_anova_int.csv")
) 
